...
class FrontEnd:
    ...
    def frames_to_magspec(self, frames):
        # compute the fft of the columns (each column is one frame; time runs
        # down axis 0), zero-padded to self.fft_size
        fft_frame = np.fft.fft(frames, self.fft_size, axis=0)
        # compute magnitude of the first fft_size//2 + 1 (positive-frequency)
        # bins of each frame, giving (num_bins, num_frames)
        magspec = np.abs(fft_frame[: self.fft_size // 2 + 1, :])
        return magspec
    ...
    def magspec_to_fbank(self, magspec):
        ...
        # Floor the energies before the log to avoid -inf on (near-)zero bins;
        # np.maximum only clamps values below eps, so real energies are unchanged.
    ...
    def mean_norm_fbank(self, fbank):
        ...
        mean_fbank = np.mean(fbank, axis=1)
        # subtract it from each frame
        fbank -= mean_fbank[:, np.newaxis]
        return fbank
