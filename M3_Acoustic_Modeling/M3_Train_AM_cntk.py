...
def create_mb_source(
    ...
    for file_name in [features_file, labels_file, label_mapping_file]:
        if not os.path.exists(file_name):
            raise RuntimeError(
                "File '{}' does not exist. Please check that datadir argument is set correctly.".format(
                    file_name
                )
            )
    ...
    fd = C.io.HTKFeatureDeserializer(
        C.io.StreamDefs(
            features=C.io.StreamDef(
                shape=feature_dim, context=context, scp=features_file
            )
        )
    )
    ...
    ld = C.io.HTKMLFDeserializer(
        label_mapping_file,
        C.io.StreamDefs(labels=C.io.StreamDef(shape=num_classes, mlf=labels_file)),
    )
...
def create_network(
    ...
    def MyMeanVarNorm(feature_mean_file, feature_inv_stddev_file):
        m = C.reshape(
            load_ascii_vector(feature_mean_file, "feature_mean"), shape=(1, feature_dim)
        )
        s = C.reshape(
            load_ascii_vector(feature_inv_stddev_file, "feature_invstddev"),
            shape=(1, feature_dim),
        )
        ...
        def _func(operand):
            return C.reshape(
                C.element_times(
                    C.reshape(operand, shape=(1 + context[0] + context[1], feature_dim))
                    - m,
                    s,
                ),
                shape=operand.shape,
    ...
    def MyDNNLayer(hidden_size=128, num_layers=2):
        return C.layers.Sequential(
            [
                C.layers.For(
                    range(num_layers),
                    lambda: C.layers.Dense(hidden_size, activation=C.sigmoid),
                )
            ]
        )
    ...
    def MyBLSTMLayer(hidden_size=128, num_layers=2):
        W = C.Parameter(
            (C.InferredDimension, hidden_size),
            init=C.he_normal(1.0),
            name="rnn_parameters",
        )

        def _func(operand):
            return C.optimized_rnnstack(
                operand,
                weights=W,
    ...
    feature_norm = MyMeanVarNorm(feature_mean_file, feature_inv_stddev_file)(
        feature_var
    )
...
def create_trainer(network, progress_writers, epoch_size):
    ...
    lr_schedule = C.learning_rate_schedule(
        lr_per_sample, unit=C.learners.UnitType.sample, epoch_size=epoch_size
    )
    mm_schedule = C.learners.momentum_as_time_constant_schedule(
        mm_time_constant, epoch_size=epoch_size
    )

    momentum_sgd_learner = C.learners.momentum_sgd(
        network["output"].parameters, lr_schedule, mm_schedule
    )

    # Create trainer
    return C.Trainer(
        network["output"],
        (network["ce"], network["pe"]),
...
def main():
    ...
    network = create_network(
        feature_dim=feature_dim,
        num_classes=num_classes,
        feature_mean_file=feature_mean_file,
        feature_inv_stddev_file=feature_inv_stddev_file,
    ...
    train_network(
        ...
        labels_file,
        label_mapping_file,
        cv_features_file=cv_features_file,
        cv_labels_file=cv_labels_file,
